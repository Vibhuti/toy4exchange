...
class login {
    ...
    private function getUserData($user_name) {
        ...
        if ($this->databaseConnection()) {
            ...
            $query_user = $this->db_connection->prepare('SELECT * FROM users WHERE UserName = :user_name');
        ...
        if (isset($_COOKIE['rememberme'])) {
            ...
            if ($hash == hash('sha256', $user_id . ':' . $token . COOKIE_SECRET_KEY) && !empty($token)) {
                ...
                if ($this->databaseConnection()) {
                    ...
                    $sth = $this->db_connection->prepare("SELECT ID, UserName, EmailID FROM users WHERE ID = :user_id
                                                      AND RememberMeToken = :user_rememberme_token AND RememberMeToken IS NOT NULL");
    ...
    private function loginWithPostData($user_name, $user_password, $user_rememberme) {
        if (empty($user_name)) {
            ...
            if (!filter_var($user_name, FILTER_VALIDATE_EMAIL)) {
                ...
                // database query, getting all the info of the selected user
                $query_user = $this->db_connection->prepare('SELECT * FROM users WHERE EmailID = :user_email');
            ...
            if (!isset($result_row->ID)) {
            ...
            } else if (($result_row->FailedLoginAttempts >= 3) && ($result_row->FailedLoginDate > (time() - 30))) {
                ...
                $sth = $this->db_connection->prepare('UPDATE users '
                        . 'SET FailedLoginAttempts = user_failed_logins+1, FailedLoginDate = :user_last_failed_login '
                        . 'WHERE UserName = :user_name OR EmailID = :user_name');
                $sth->execute(array(':user_name' => $user_name, ':user_last_failed_login' => time()));

                $this->errors[] = MESSAGE_PASSWORD_WRONG;
                // has the user activated their account with the verification email
            } else if ($result_row->IsActive != 1) {
                ...
                $sth = $this->db_connection->prepare('UPDATE users '
                        . 'SET FailedLoginAttempts = 0, FailedLoginDate = NULL '
                        . 'WHERE ID = :user_id AND FailedLoginAttempts != 0');
                ...
                if (defined('HASH_COST_FACTOR')) {
                    ...
                    if (password_needs_rehash($result_row->Password, PASSWORD_DEFAULT, array('cost' => HASH_COST_FACTOR))) {
                        ...
                        $query_update = $this->db_connection->prepare('UPDATE users SET Password = :user_password_hash WHERE ID = :user_id');
                        $query_update->bindValue(':user_password_hash', $user_password_hash, PDO::PARAM_STR);
                        $query_update->bindValue(':user_id', $result_row->ID, PDO::PARAM_INT);
                    ...
                    //Set Cookie expiration to 30 days and redirect the page back to index.php
                    ...
                    $home_url = 'http://localhost:90/Demo/index.php';
    ...
    private function newRememberMeCookie() {
        ...
        if ($this->databaseConnection()) {
            ...
            $sth = $this->db_connection->prepare("UPDATE users SET RememberMeToken = :user_rememberme_token WHERE ID = :user_id");
    ...
    private function deleteRememberMeCookie() {
        ...
        if ($this->databaseConnection()) {
            ...
            $sth = $this->db_connection->prepare("UPDATE users SET RememberMeToken = NULL WHERE ID = :user_id");
    ...
    public function editUserName($user_name) {
        ...
        if (!empty($user_name) && $user_name == $_SESSION['user_name']) {
            ...
            if (isset($result_row->ID)) {
                ...
                $query_edit_user_name = $this->db_connection->prepare('UPDATE users SET UserName = :user_name WHERE ID = :user_id');
    ...
    public function editUserEmail($user_email) {
        ...
        if (!empty($user_email) && $user_email == $_SESSION["user_email"]) {
            ...
            if (isset($result_row->ID)) {
                ...
                $query_edit_user_email = $this->db_connection->prepare('UPDATE users SET EmailID = :user_email WHERE ID = :user_id');
    ...
    public function editUserPassword($user_password_old, $user_password_new, $user_password_repeat) {
        if (empty($user_password_new) || empty($user_password_repeat) || empty($user_password_old)) {
            ...
            if (isset($result_row->Password)) {
                ...
                // using PHP 5.5's password_verify() function to check if the provided passwords fits to the hash of that user's password
                if (password_verify($user_password_old, $result_row->Password)) {
    ...
    public function setPasswordResetDatabaseTokenAndSendMail($user_name) {
        ...
        if (empty($user_name)) {
            ...
            if (isset($result_row->ID)) {
                ...
                // database query:
                $query_update = $this->db_connection->prepare('UPDATE users SET PasswordResetToken = :user_password_reset_hash,
                                                               PasswordResetTime = :user_password_reset_timestamp
                ...
                if ($query_update->rowCount() == 1) {
                    ...
                    $this->sendPasswordResetMail($user_name, $result_row->EmailID, $user_password_reset_hash);
                    return true;
                } else {
    ...
    public function checkIfEmailVerificationCodeIsValid($user_name, $verification_code) {
        ...
        if (empty($user_name) || empty($verification_code)) {
            ...
            if (isset($result_row->ID) && $result_row->PasswordResetToken == $verification_code) {

                $timestamp_one_hour_ago = time() - 3600; // 3600 seconds are 1 hour

                if ($result_row->PasswordResetTime > $timestamp_one_hour_ago) {
    ...
    public function editNewPassword($user_name, $user_password_reset_hash, $user_password_new, $user_password_repeat) {
        ...
        if (empty($user_name) || empty($user_password_reset_hash) || empty($user_password_new) || empty($user_password_repeat)) {
            ...
            // write users new hash into database
            $query_update = $this->db_connection->prepare('UPDATE users SET Password = :user_password_hash,
                                                           PasswordResetToken = NULL, PasswordResetTime = NULL
                                                           WHERE UserName = :user_name AND PasswordResetToken = :user_password_reset_hash');
            $query_update->bindValue(':user_password_hash', $user_password_hash, PDO::PARAM_STR);
            $query_update->bindValue(':user_password_reset_hash', $user_password_reset_hash, PDO::PARAM_STR);
            $query_update->bindValue(':user_name', $user_name, PDO::PARAM_STR);
            $query_update->execute();
